...
The sequence is simple: next-token prediction, then chat assistance, then tool-using systems for multi-step tasks.
...
For coding agents, search, read, inspect, and repeat is also a way to manage context.
:::
...
What changes across these patterns is how long the system keeps control.
:::
...
In real projects, the gain is targeted retrieval instead of one oversized prompt.
...
What changes across modes is how long Copilot keeps control: a short answer, a bounded edit, or a longer task loop.
...
If your colleagues think in RStudio terms, Positron is the easiest next step.
...
- The extra context comes from the live session: attached `Interpreter Sessions…`, Console inputs and outputs, in-memory objects in the `Variables Pane`, and the active plot in the `Plots Pane`.
- From the paperclip menu, you can attach `Files & Folders`, `Interpreter Sessions…`, `Source Control…`, and `Tools`.
- It is a good fit for exploratory R analysis, Quarto, and Shiny because the agent can stay tied to the live session.

## Copilot CLI

Copilot CLI gives the agent the most freedom.

- It is the most general surface for file access, shell commands, logs, renders, builds, git, and long-running tasks.
- Once you trust a folder, it can read, modify, and execute within that directory tree. This is the least constrained surface.
- It is a strong fit for `renv`, `targets`, Quarto renders, shell-heavy workflows, and repository maintenance.
- It surfaces the agent plumbing directly: autopilot mode, trusted directories, tool approvals, custom agents, skills, hooks, and MCP.
- It gets work done quickly, but terminal review is weak. After a substantial task, switch back to the IDE for diff review and cleanup.

## Constructing closed feedback loops

Agents work best when the task can check its own output.

- Use a tight loop: `edit -> run -> inspect -> fix`.
...
- Custom instructions give reusable repository or team guidance.
- Skills bundle task-specific instructions, scripts, or resources.
- Subagents or custom agents handle bounded helper roles.
- Hooks run checks before or after key events.

The names differ across products, but the job is the same: instructions shape behavior, skills add know-how, subagents split work, and hooks enforce process.
...
- They are still weak at abstraction design and at noticing when a copied local pattern is the wrong one.
...
- The main gain here is faster iteration on research tasks that still have a clear human owner.
...
- Find a workflow that fits your work, then keep refining it.
